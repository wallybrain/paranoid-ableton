...
- `/home/user/n8n-mcp-server/index.js` -- User's MCP server using SDK v1.25.3 with low-level Server API, verified import paths and error handling pattern
- `/home/user/epistemic-mcp/src/index.js` -- User's MCP server using SDK v0.5.0, verified same API pattern works across versions
- `/home/user/n8n-mcp-server/node_modules/@modelcontextprotocol/sdk/` -- Examined v1.25.3 SDK package.json exports, Server types, McpServer types, StdioServerTransport, CallToolResultSchema
- `/home/user/epistemic-mcp/node_modules/@modelcontextprotocol/sdk/` -- Examined v0.5.0 SDK structure confirming different dist layout but compatible import paths
- `/home/user/ableton-mcp/src/osc-client.js` -- Phase 1 OscClient with healthCheck(), classifyError(), open(), close(), query()
- `/home/user/ableton-mcp/test/osc-client.test.js` -- Phase 1 test patterns with MockUDPPort

### Secondary (MEDIUM confidence)
- `/home/user/.claude.json` -- Verified Claude Code MCP server registration format for stdio servers

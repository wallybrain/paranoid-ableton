...
Even PR #183's `/live/browser/load_sample` searches Ableton's **internal browser index** by name, NOT by filesystem path. A sample in `/home/user/samples/kicks/kick_01.wav` would only be found if that directory is already added to Ableton's browser Places and the name matches.
...
1. **User's sample directory locations**
   ...
   - Recommendation: `sample_scan` accepts explicit directory paths. Document common Ableton sample locations (`~/Music/Ableton/`, `/home/user/Music/`, Ableton User Library) in tool description.
